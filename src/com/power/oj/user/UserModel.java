...
public class UserModel extends Model<UserModel>
{
  ...
  {
    return findFirst("SELECT uid,name,nick,avatar,school,blog,online,shareCode,gender,submission,solved,loginTime,ctime,signature,comeFrom FROM user WHERE name=? LIMIT 1", name);
  }

  public UserModel getUserInfoByUid(Integer uid)
  {
    return findFirst("SELECT uid,name,nick,avatar,school,blog,online,shareCode,gender,submission,solved,loginTime,ctime,signature,comeFrom FROM user WHERE uid=? LIMIT 1", uid);
  }
  ...
  public int getUserRank(Integer uid)
  {
    ...
    Object object = findFirst(
        "SELECT rank FROM (SELECT @rank:=@rank+1 AS rank,uid FROM user,(SELECT @rank:=0)r ORDER BY solved desc,submission)t_rank WHERE uid=? LIMIT 1", uid).get(
        "rank");
  ...
  {
    Page<UserModel> userList = paginate(pageNumber, pageSize, "SELECT @rank:=@rank+1 AS rank,uid,name,nick,realName,solved,submission",
        "FROM user,(SELECT @rank:=?)r WHERE status=1 ORDER BY solved DESC,submission,uid", (pageNumber - 1) * pageSize);
