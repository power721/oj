    ...
    <form class="form-horizontal" id="createContest" action="cprogram/update/${contest.cid}" method="post">
        <div class="control-group contest <#if titleMsg??>error</#if>">
            <label class="control-label" for="inputTitle">
                ${contestTypeMap.get(contest.type)}内容
            </label>
            <div class="controls">
                <textarea id="inputTitle" name="title" rows="1" required
                          style="width: 246px">${contest.title!}</textarea>
                            ...
                            <#if techerList??>
                                <#list techerList as teacher>
                                    <option value="${teacher.uid!}"
                                            <#if teacher.uid == contest.uid>selected="selected"</#if> > ${teacher.realName!}
                                    </option>
                                </#list>
                            </#if>
                        </select>
                    ...
                    <div class="input-prepend">
                        <select name="week" id="class_week">
                            <#list weeksMap.keySet() as week>
                                <option value="${week!}"
                                        <#if week == contest.week>selected="selected"</#if> >${weeksMap.get(week!)}
                                </option>
                            </#list>
                        </select>
                        <select name="lecture" id="class_lecture">
                            <#list lecturesMap.keySet() as lecture>
                                <option value="${lecture!}"
                                        <#if lecture == contest.lecture>selected="selected"</#if> >
...
<@override name="scripts">
    ...
    <script type="text/javascript">
        ...
        $('#startTime').datetimepicker({
            ...
            <#if contest.type == "HOMEWORK" || contest.type == "EXPERIMENT">
