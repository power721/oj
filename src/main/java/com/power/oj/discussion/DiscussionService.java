...
public final class DiscussionService {
	...
	public List<TopicModel> getTopicTree(TopicModel topic) {
		...
		topicList.add(topic);
		List<TopicModel> children = dao
				.find("SELECT t.*,u.name FROM `topic` t LEFT JOIN `user` u ON u.uid=t.uid WHERE t.parentId=? ORDER BY t.orderNum DESC,t.id",
						topic.getId());
	...
	public TopicModel findTopic4Show(Integer id) {
		TopicModel topic = dao.findFirst("SELECT t.*,p.title AS problem,u.name FROM `topic` t "
				+ "LEFT JOIN problem p ON p.pid=t.pid " + "LEFT JOIN user u ON u.uid=t.uid WHERE t.id=?", id);
