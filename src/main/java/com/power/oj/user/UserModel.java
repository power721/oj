...
public class UserModel extends Model<UserModel> {
   ...
   */
	...
	public Page<UserModel> getUserRankList(int pageNumber, int pageSize) {
		Page<UserModel> userList = paginate(pageNumber, pageSize,
				"SELECT @rank:=@rank+1 AS rank,uid,name,nick,realName,solved,submission",
	 ...
	 * "SELECT pid FROM solution WHERE uid=? AND result!=0 AND status=1 GROUP BY pid"
	 * , uid); }
	 */
