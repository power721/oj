...
public class UserController extends OjController {
    ...
    public void rank() {
        ...
        int pageSize = getParaToInt("pageSize", OjConfig.userPageSize);
    ...
    public void login() {
        if (ShiroKit.isAuthenticated()) {
            FlashMessage msg =
                new FlashMessage(getText("user.error.login"), MessageType.WARN, getText("message.warn.title"));
    ...
    public void forget() {
        if (!ojService.checkEmailConf()) {
            FlashMessage msg =
                new FlashMessage(getText("conf.email.error"), MessageType.ERROR, getText("message.error.title"));
            redirect(sessionService.getLastAccessURL(), msg);
        }
