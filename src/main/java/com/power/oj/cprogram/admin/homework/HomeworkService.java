...
public class HomeworkService {
    static public List<Map<String, Object>> getAllScore(Integer tid, Integer week, Integer lecture) {
        ...
        List<ScoreModel> scoreModelList = ScoreModel.dao.find(
                "SELECT uid,cid,score2 from score s WHERE cid in (SELECT c.cid FROM contest c INNER JOIN cprogram_info ci ON c.cid=ci.cid WHERE c.uid=? AND c.type=999 AND ci.type=? AND startTime BETWEEN ? AND ? AND ci.`week`=? AND ci.lecture=?)",
                tid, "HOMEWORK", startTime, endTime, week, lecture);
    ...
    static public List<ContestModel> getHomeWorkList(Integer tid, Integer week, Integer lecture) {
        int startTime = CProgramService.getStartUnixTime();
        int endTime = CProgramService.getEndUnixTime();
        return ContestModel.dao.find("SELECT c.cid FROM contest c INNER JOIN cprogram_info ci ON c.cid=ci.cid WHERE c.uid=? AND c.type=999 AND ci.type=? AND startTime BETWEEN ? AND ? AND ci.`week`=? AND ci.lecture=?",
                tid, "HOMEWORK", startTime, endTime, week, lecture);
    }

    static public File getHomeworkXls(Integer tid, Integer week, Integer lecture) throws IOException, WriteException {
        File file;
        ...
        List<ContestModel> contestList = getHomeWorkList(tid, week, lecture);
