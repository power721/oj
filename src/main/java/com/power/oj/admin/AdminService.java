...
public final class AdminService {
		...
		for (String pidPar : pidStr) {
			...
			if (pids.length == 1) {
				try {
					...
					if (status) {
						problemModel = ProblemModel.dao.findFirst("SELECT * FROM problem WHERE pid=? AND status=1", pid);
					} else {
				...
				if (start <= end) {
					...
					if (status) {
						problemList = ProblemModel.dao.find("SELECT * FROM problem WHERE pid>=? AND pid<=? AND status=1", start,
								end);
					} else {
						problemList = ProblemModel.dao.find("SELECT * FROM problem WHERE pid>=? AND pid<=?", start, end);
					}
	...
	public File exportProblems(Integer start, Integer end, Boolean status, Boolean replace) {
		...
		List<ProblemModel> problemList = ProblemModel.dao.find("SELECT * FROM problem WHERE pid>=? AND pid<=? AND status=?",
				start, end, status ? 1 : 0);
		for (ProblemModel problemModel : problemList) {
			Element item = rootElement.addElement("item");
			item.addAttribute("pid", String.valueOf(problemModel.getPid()));
			FpsProblem problem = new FpsProblem(problemModel);
