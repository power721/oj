...
public final class SocialService {
	...
	public List<FriendGroupModel> getGroupList(Integer uid) {
		List<FriendGroupModel> groupList = dao
				.find("(SELECT g.id,g.uid,g.name,COUNT(f.userId) AS count,g.ctime FROM friend_group g LEFT JOIN friend f ON f.userId=? AND f.gid=g.id WHERE g.uid=0) UNION (SELECT * FROM friend_group WHERE uid=? ORDER BY id)",
						uid, uid);
