...
public final class SocialService {
    ...
    public List<FriendGroupModel> getGroupList(Integer uid) {
        List<FriendGroupModel> groupList = dao.find(
    ...
    public Page<FriendGroupModel> getFollowingList(int pageNumber, int pageSize, Integer uid, Integer gid) {
        String sql =
            "SELECT g.name AS groupName,f.*,u.name,u.solved,u.submission,u.comeFrom,u.gender,u.signature,u.avatar,(SELECT 1 FROM friend ff WHERE ff.userId=f.friendUid AND ff.friendUid=f.userId) AS isFriend";
        String from = null;

        if (gid == null || gid <= 0) {
            gid = 0;
            from =
                "FROM friend f LEFT JOIN friend_group g ON f.gid=g.id LEFT JOIN user u ON u.uid=f.friendUid WHERE f.userId=? AND f.gid>?";
        } else {
            from =
                "FROM friend f LEFT JOIN friend_group g ON f.gid=g.id LEFT JOIN user u ON u.uid=f.friendUid WHERE f.userId=? AND f.gid=?";
        }
    ...
    public Page<FriendGroupModel> getFollowedList(int pageNumber, int pageSize, Integer uid) {
        String sql =
            "SELECT g.name AS groupName,f.*,u.name,u.solved,u.submission,u.comeFrom,u.gender,u.signature,u.avatar,(SELECT 1 FROM friend ff WHERE ff.userId=f.friendUid AND ff.friendUid=f.userId) AS isFriend";
        String from =
            "FROM friend f LEFT JOIN friend_group g ON f.gid=g.id LEFT JOIN user u ON u.uid=f.userId WHERE f.friendUid=?";
    ...
    public boolean changeFollowingByGroup(Integer uid, Integer fid, Integer gid, Integer tgid) {
        ...
        boolean result =
            Db.update("UPDATE friend SET gid=? WHERE userId=? AND friendUid=? AND gid=?", tgid, uid, fid, gid) > 0;
        if (result) {
            if (gid > 1) {
