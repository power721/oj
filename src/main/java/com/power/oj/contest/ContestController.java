...
public class ContestController extends OjController {
	...
	public void problem() {
		...
		if (problemModel == null) {
			FlashMessage msg = new FlashMessage(getText("contest.problem.null"), MessageType.ERROR,
	...
	public void submit() {
		...
		if (contestService.isContestFinished(cid)) {
			FlashMessage msg = new FlashMessage(getText("contest.submit.finished"), MessageType.WARN,
		...
		setTitle(new StringBuilder(6).append(getText("contest.problem.title")).append(cid).append("-").append(id).append(": ")
				.append(problemModel.getTitle()).toString());
		if (ajax)
	...
	public void code() {
		...
		if (!uid.equals(loginUid) && !isAdmin) {
			FlashMessage msg = new FlashMessage(getText("solution.show.error"), MessageType.ERROR,
					getText("message.error.title"));
			redirect("/status", msg);
			return;
		}
	...
	public void updateProblem() {
		...
		String redirectURL = new StringBuilder(4).append("/contest/problem/").append(cid).append("-").append((char) (num + 'A'))
				.toString();
	...
	public void admin() {
		...
		// setTitle(new
		// StringBuilder(2).append(getText("contest.admin.title")).append(cid).toString());
	}
	...
	public void build() {
		...
		redirect(new StringBuilder(2).append("/contest/rank/").append(cid).toString(), new FlashMessage(
				getText("contest.buildRank.success")));
	}
