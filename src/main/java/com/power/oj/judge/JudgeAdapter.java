...
public abstract class JudgeAdapter implements Runnable {
	...
	protected void prepare() throws IOException {
		...
		if (solution instanceof SolutionModel && !needDeleteTempDir() && OjConfig.getBoolean("deleteTmpFile", false)) {
			File prevWorkDir = new File(new StringBuilder(2).append(workPath).append(solution.getSid() - RESERVED_TEMP_DIRS)
					.toString());
		...
		File sourceFile = new File(new StringBuilder(5).append(workDirPath).append(File.separator)
