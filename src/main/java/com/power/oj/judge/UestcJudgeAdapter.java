...
public class UestcJudgeAdapter extends JudgeAdapter {
	...
	private int convertResult(int result) {
		/*
		 * const int OJ_WAIT = 0; //Queue const int OJ_AC = 1; //Accepted const
		 * int OJ_PE = 2; //Presentation Error const int OJ_TLE = 3; //Time
		 * Limit Exceeded const int OJ_MLE = 4; //Memory Limit Exceeded const
		 * int OJ_WA = 5; //Wrong Answer const int OJ_OLE = 6; //Output Limit
		 * Exceeded const int OJ_CE = 7; //Compilation Error const int
		 * OJ_RE_SEGV = 8; //Segment Violation const int OJ_RE_FPE = 9; //FPU
		 * Error const int OJ_RE_BUS = 10;//Bus Error const int OJ_RE_ABRT =
		 * 11;//Abort const int OJ_RE_UNKNOWN = 12;//Unknow const int OJ_RF =
		 * 13;//Restricted Function const int OJ_SE = 14;//System Error const
		 * int OJ_RE_JAVA = 15;//JAVA Run Time Exception
		 */
		switch (result) {
		case 0:
