...
public class ContestApiController extends OjController {
	...
	public void problemStatus() {
		...
		Page<ContestSolutionModel> solutionList = solutionService.getProblemStatusPageForContest(pageNumber, pageSize, language,
				cid, num);
	...
	public void code() {
		...
		if (!isAdmin) {
			...
			if (error != null) {
				solutionModel.setError(error.replace(StringUtil.replace(OjConfig.getString("workPath"), "\\", "\\\\"), ""));
				// TODO replace "/"
			}
		}
		...
		renderJson(new String[] { "success", "alpha", "problemTitle", "language", "resultLongName", "resultName", "solution",
				"brush" });
	}
